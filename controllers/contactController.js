...
const createContact = asyncHandle(async(req,res)=>{
    try {
        ...
        const newBlog = await contactModel.create(req.body);
        res.status(200).send({
            success: true,
...
const updateContact = asyncHandle(async (req, res) => {
    ...
    try {
      ...
      const updateblog = await contactModel.findOneAndUpdate({ _id }, req.body.blog, {
        new: true,
      })
      res.status(200).send({
        success : true,
...
const deleteContact = asyncHandle(async (req, res) => {
    const {_id} = req.params;
    try {
      const deleteContact = await contactModel.findOneAndDelete({_id})
      res.status(200).send({
        success : false,
...
// get a product
const getaContact = asyncHandle(async(req,res)=>{
    const {id}= req.params
    try {
        const findBlog = await contactModel.findById(id).populate("orderBy")
        res.status(200).send({
            success : true,
...
// get all product
const getAllContact = asyncHandle(async(req,res)=>{
    try {
        ...
        let query = contactModel.find(JSON.parse(queryStr)) // tìm kiếm các từ khóa được trả về
        ...
        if(req.query.page){
            const productCount = await contactModel.countDocuments()
...
module.exports = {createContact ,getaContact,getAllContact,updateContact,deleteContact
  }
