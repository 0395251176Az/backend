const priceRoomModel = require('../models/priceRoomModel')
const asyncHandle = require('express-async-handler')


// create new product
const createPrice = asyncHandle(async(req,res)=>{
    try {
        const newBlog = await priceRoomModel.create(req.body);
        res.status(200).send({
            success: true,
...
// update product
const updatePrice = asyncHandle(async (req, res) => {
    const {_id} = req.params;
    try {
      
      const updateblog = await priceRoomModel.findOneAndUpdate({ _id }, req.body.blog, {
        new: true,
      })
      res.status(200).send({
        success : true,
  ...
  // delete product
const deletePrice = asyncHandle(async (req, res) => {
    const {_id} = req.params;
    try {
      const deletePrice = await priceRoomModel.findOneAndDelete({_id})
      res.status(200).send({
        success : false,
...
// get a product
const getaPrice = asyncHandle(async(req,res)=>{
    const {id}= req.params
    try {
        const findBlog = await priceRoomModel.findById(id)
        res.status(200).send({
            success : true,
...
// get all product
const getAllPrice = asyncHandle(async(req,res)=>{
    try {
        // filtering 
        ...
        let query = priceRoomModel.find(JSON.parse(queryStr)) // tìm kiếm các từ khóa được trả về
        ...
        if(req.query.page){
            const productCount = await priceRoomModel.countDocuments()
...
module.exports = {createPrice ,getaPrice,getAllPrice,updatePrice,deletePrice
  }
